-- Allow half-star ratings (1.0–5.0 in 0.5 steps)

alter table public.escape_session_details
  drop constraint if exists escape_session_details_rating_check;
...
alter table public.escape_session_details
  add constraint escape_session_details_rating_check
  check (
    rating is null
    or (
      rating >= 1
...
comment on column public.escape_session_details.rating is
  'Personal 1–5 star rating (0.5 steps) for the escape room experience.';
