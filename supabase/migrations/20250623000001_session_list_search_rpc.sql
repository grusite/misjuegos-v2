-- Server-side session list filters (search + participants) — avoids PostgREST nested or() limits

create index if not exists play_sessions_played_at_desc_idx
  on public.play_sessions (played_at desc);

create index if not exists session_participants_participant_session_idx
  on public.session_participants (participant_id, session_id);

create index if not exists play_sessions_player_team_id_idx
  on public.play_sessions (player_team_id)
  where player_team_id is not null;

create or replace function public.list_play_session_summaries(
...
grant execute on function public.list_play_session_summaries to authenticated;
